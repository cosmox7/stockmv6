...
let _catalog = {};   // in-memory cache rebuilt from DB
...
async function addLog(cat,prod,type,qty,bal,amt,note){
  const h=await getHistory(),d=today();if(!h[d])h[d]=[];
  h[d].push({cat,product:prod,code:codeOf(prod),type,qty,balance:bal,salePrice:amt||null,note:note||"",time:new Date().toLocaleTimeString("hi-IN",{hour:"2-digit",minute:"2-digit"})});
  await setHistory(h);
}
...
async function submitSale(){
  ...
  const nq=cur-qty;s[prod]=nq;await setStock(s);
...
async function savePricing(){
  ...
  const p=await getPricing();p[prod]={mrp,discountPrice:!isNaN(disc)&&disc>0?disc:null,discountPct:!isNaN(disc)&&disc>0?parseFloat(((mrp-disc)/mrp*100).toFixed(2)):0,note:note||"",updatedAt:new Date().toLocaleString("hi-IN")};
  await setPricing(p);showMsg("priceMsg",`✅ ${prod} — MRP: ${fmt(mrp)}`,false);showToast("Pricing saved");await renderDash();renderPricingTable();
}
...
async function delPricing(n){if(!confirm(`"${n}" ki pricing delete karna chahte ho?`))return;const p=await getPricing();delete p[n];await setPricing(p);renderPricingTable();await renderDash();showToast("Pricing removed")}
...
async function saveExpiry(){
  ...
  const entries=await getExpiry();entries.push({id:Date.now(),cat,product:prod,code:codeOf(prod),batch:batch||"",qty,originalQty:qty,expDate:date,status:"active",addedOn:today()});
  await setExpiry(entries);showMsg("expMsg",`✅ Batch saved! ${prod} — ${qty} units — Exp: ${fmtD(date)}`,false);showToast(`Batch added: ${prod}`);
...
async function saveExpEdit(){
  ...
  const entries=await getExpiry(),idx=entries.findIndex(e=>e.id===id);if(idx!==-1){entries[idx].qty=qty;await setExpiry(entries)}
  closeExpEdit();await renderExpList();await renderExpSummary();showToast("Qty updated");
}
async function disposeExp(id){if(!confirm("Batch disposed mark karna chahte ho?"))return;const entries=await getExpiry(),idx=entries.findIndex(e=>e.id===id);if(idx!==-1){entries[idx].status="disposed";entries[idx].disposedOn=today();await setExpiry(entries)}await renderExpList();await renderExpSummary();showToast("Batch disposed")}
async function delExp(id){if(!confirm("Entry delete karna chahte ho?"))return;await setExpiry((await getExpiry()).filter(e=>e.id!==id));await renderExpList();await renderExpSummary();showToast("Entry deleted")}
...
async function applyBulkPricing(){
  ...
  await setPricing(p);showMsg("bulkPricingMsg",`✅ ${updated} products updated with ${pct}%`,false);showToast(`Bulk pricing: ${cat}`);renderPricingTable();await renderDash();
}
async function resetCatPricing(){
  ...
  await setPricing(p);showMsg("resetPricingMsg","✅ Reset done!",false);renderPricingTable();await renderDash();showToast("Pricing reset");
}
...
async function doReset(type){
  ...
  if(type==="history")await setHistory({});
  else if(type==="stock")await setStock({});
  else if(type==="pricing")await setPricing({});
  else if(type==="expiry")await setExpiry([]);
  else if(type==="catalog"){await dDel(KEY.CATALOG);await dDel(KEY.INITDONE);_catalog={};rebuildPROD();await refreshAllDropdowns();location.reload()}
  else if(type==="everything"){if(!confirm("Pakka sure ho? Yeh undo nahi hoga!"))return;await setStock({});await setHistory({});await setPricing({});await setExpiry([])}
...
async function saveCatModal(){
  ...
  await setCatalogData(_catalog);rebuildPROD();await refreshAllDropdowns();closeCatModal();await renderCatList();if(_sCat)selCat(_sCat);showMsg("catalogMsg",`✅ "${newN}" saved!`,false);showToast("Catalog saved ✅");
}
...
async function checkAppLock(force = false) {
  ...
  if (!force && val.length < 4) return; // wait for enough chars
  if (!force && val.length < (await getAppPassword()).length) return;
  ...
  } else if (val.length >= correct.length || force) {
    // Wrong
